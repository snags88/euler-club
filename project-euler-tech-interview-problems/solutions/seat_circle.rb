...
def last_person(seats)
  ...
  while seats.length > 1 
    seats.each.with_index do |seat, index|
      if skip
        skip = false
      else
        seats.delete(seat)
...
seats = (1..100).to_a
puts "\nseats = ["+seats.join(", ")+"]\n"
puts "\nseats.length = "+seats.length.to_s
puts "\nlast_person(seats) => "+last_person(seats).to_s

puts "\nNow with Flatiron peeps...\n"
sleep(3)

seats = ["Avi", "Rose", "Uzo", "Ian", "Josh", "Amanda", "Alex", "Alibaba512", "Becca", "Bethanne", "Chloe", "Dan", "Ian", "Jeremy", "Jennifer", "Kate", "Lara", "Rachel", "Seiji", "Skylar", "Ville", "Andrew", "Casie", "Cyrus", "Deniz", "Jason", "Rebecca", "Sophie"]
results_index = seats.find_index("Casie")
puts "\nseats = ['"+seats.join("', '")+"]\n"
puts "\nseats.length = "+seats.length.to_s
puts "\nlast_person(seats) => "+"'"+last_person(seats).to_s+"'"
puts "Last person's index position: "+results_index.to_s
